...
5. **AI usage documentation is worth 30% of the oral report (≈18% of the total grade).**
   This is the single highest-leverage non-obvious scoring item. After **every meaningful change**, append an entry to the current-month file in [docs/log/](docs/log/) (large = 10-field template, small = one-liner). Format is defined in [docs/CLAUDE.md §Log 寫法](docs/CLAUDE.md#log-寫法).
