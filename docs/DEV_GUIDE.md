...
| `K` / `L` | 顏色數 − / + |
| `Tab` | 切換目前編輯的顏色 |
| `Up` / `Down` | 選擇要編輯的列/欄數字格 |
| `Left` / `Right` | 選中數字 − / + |
| 滑鼠左鍵（PART DESIGNER 網格） | 點格 toggle，畫出零件形狀 |
| `A` | 把畫好的形狀加入為一個零件（目前顏色） |
| `D` | 刪除最後加入的零件 |
| `E` | 匯出成 `assets/levels/custom-N.txt` |
| `P` | 用目前設定直接試玩 |
| `Esc` | 回主選單 |

> 已含：大小/顏色數/列欄數字 + **任意形狀零件設計器** + 匯出 + 試玩。點格畫「不可放置格 / 固定零件」為後續。
