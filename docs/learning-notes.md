...
編輯流程不是手動填每列/欄的數字，而是**把零件擺到盤面排出一個解答**，再由 `Editor::deriveConstraints()` 用 `Board::currentFilledForColor` 把每色每列/欄的填滿數算出來當需求（固定格也計入）。匯出時零件回到 tray（未放置），數字＝你排的佈局。

關鍵正確性論點：因為數字正是某個**完整合法佈局**的行列計數，這個關卡**保證至少有一個解**（就是你排的）。手動填數字則可能填出無解的題目。這也是為什麼 `placePart` 用 `Board::canPlace`、resize/換色時清掉擺放保持一致——編輯期間的盤面始終是一個合法狀態。
